...
class PointTR
{
  public:
    ...
    {
      return colloc.idxmap(j1, j2, i1, i2);
    }
    ...
    inline void    ImportSol(Vector& Sol)
    {
    ...
    inline double  Norm()
    {
      return sqrt(colloc.Integrate(xx->getV1(), xx->getV1()));
    }
    inline void    SaveSol(const char* dat, const char* idx)
    {
      colloc.Save(dat, idx, xx->getV1());
    }
    inline void    SaveTan(const char* dat, const char* idx)
    {
      colloc.Save(dat, idx, xxDot->getV1());
    }
