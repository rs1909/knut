...
In order to install the software one needs a Unix like environment with a shell program. For compiling the package it is recommended to use the GNU C and C++ compilers of versions 3.3.6 and later. Although Sun Studio 10, and the Intel C,C++ (v. 8.1 or later) compilers can build the package, they are not guaranteed to work with future versions of the package. Furthermore, the \filef{sed} command is needed, when using the \filef{pcompile} script.
