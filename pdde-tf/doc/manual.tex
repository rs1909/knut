...
$ ./build-unix.sh
\end{lstlisting} \end{quote} } \noindent
This configures and builds software after which it install into the \filef{pdde-cont/} folder in the user's home directory.
...
If you install the binary version of the software, the only requirement is to install MinGW, which is necessary to compile system definition files. In this case just uncompress the \filef{pdde-windows-{\pddever}.zip} and it is ready to use.

Installing from source requires a bit more work. First download 
the most recent self extracting installer of MinGW from \url{http://www.mingw.org/} (at the time of writing 5.0.3) and install it. The installer will ask you to select options. Tick the boxes of the C,C++ compiler and the GNU make utility, the other components are not required. Download and install the Windows version of CMake from \url{www.cmake.org} and a very recent version of Qt (4.2.0 and later) from \url{http://www.trolltech.com/developer/downloads/qt/windows}.
...
{ \small \begin{quote} \begin{lstlisting}[basicstyle=\tt,frame=single]
 > build-windows
\end{lstlisting} \end{quote} } \noindent
command, which will build and install the software into \filef{Program Files$\backslash$PDDE}. The program then can be started from the \filef{bin} subdirectory. Note that no ``Start menu'' entries, no shortcuts are made to the software, so these steps have to be performed manually if required. To uninstall, simply delete the PDDE directory.
...
Note: The code of UMFPACK was \emph{not} modified, only its build system was rewritten to use CMake.
...
Note: The code of AMD was \emph{not} modified, only its build system was rewritten to use CMake.
