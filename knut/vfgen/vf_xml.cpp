...
void VectorField::PrintXML(std::ostream& outstr, const std::string& cmdstr)
{
  outstr << "<?xml version=\"1.0\" ?>" << endl;
  outstr << "<!--\n";
  outstr << "  This file was generated by the program VFGEN (Version: " << VERSION << ") with the " << cmdstr << " command.\n";
  outstr << "  " << DateTimeMsg() << endl;
  outstr << "-->\n";
  outstr << "<VectorField";
  outstr << " Name=\"" << Name() << "\"";
  if (Description() != "")
  ...
  outstr << " IndependentVariable=\"" << IndependentVariable << "\"";
  outstr << ">" << endl;
  ...
  for (c = Constants.begin(); c != Constants.end(); ++c)
  {
    ...
    if ((*c)->Description() != "")
    {
      outstr << " Description=\"" << (*c)->Description() << "\"";
    }
    outstr << " Value=\"" << (*c)->Value() << "\"";
    if ((*c)->Latex() != "")
    {
      outstr << " Latex=\"" << (*c)->Latex() << "\"";
    }
    outstr << "/>" << endl;
  }
  ...
  for (p = Parameters.begin(); p != Parameters.end(); ++p)
  {
    if ((*p)->Description() != "__internal__")
    {
      ...
      if ((*p)->Description() != "")
      {
        outstr << " Description=\"" << (*p)->Description() << "\"";
      }
      outstr << " DefaultValue=\"" << (*p)->DefaultValue() << "\"";
      if ((*p)->Latex() != "")
      {
        outstr << " Latex=\"" << (*p)->Latex() << "\"";
      }
      outstr << "/>" << endl;
    }
  }
  ...
  for (e = Expressions.begin(); e != Expressions.end(); ++e)
  {
    ...
    if ((*e)->Description() != "")
    {
      outstr << " Description=\"" << (*e)->Description() << "\"";
    }
    outstr << " Formula=\"" << (*e)->Formula() << "\"";
    if ((*e)->Latex() != "")
    {
      outstr << " Latex=\"" << (*e)->Latex() << "\"";
    }
    outstr << "/>" << endl;
  }
  ...
  {
    outstr << "<StateVariable";
    outstr << " Name=\"" << (*sv)->Name() << "\"";
    if ((*sv)->Description() != "")
    {
      outstr << " Description=\"" << (*sv)->Description() << "\"";
    }
    outstr << " Formula=\"" << (*sv)->Formula() << "\"";
    if ((*sv)->PeriodicFrom() != "")
    {
      outstr << " PeriodFrom=\"" << (*sv)->PeriodicFrom() << "\"";
    }
    if ((*sv)->PeriodicTo() != "")
    {
      outstr << " PeriodTo=\"" << (*sv)->PeriodicTo() << "\"";
    }
    if ((*sv)->DefaultInitialCondition() != "")
    {
      outstr << " DefaultInitialCondition=\"" << (*sv)->DefaultInitialCondition() << "\"";
    }
    if ((*sv)->DefaultHistory() != "")
    {
      outstr << " DefaultHistory=\"" << (*sv)->DefaultHistory() << "\"";
    }
    if ((*sv)->Mass() != "")
    {
      outstr << " Mass=\"" << (*sv)->Mass() << "\"";
  ...
  for (f = Functions.begin(); f != Functions.end(); ++f)
  {
    ...
    if ((*f)->Description() != "")
    {
      outstr << " Description=\"" << (*f)->Description() << "\"";
    }
    outstr << " Formula=\"" << (*f)->Formula() << "\"";
    outstr << "/>" << endl;
  }

  outstr << "</VectorField>" << endl;
