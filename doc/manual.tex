...
The recommended way of compiling the package is to use the GNU toolchain, the GNU C and C++ compilers and the GNU make utility. The recommended versions of gcc are 3.2.3 or newer.
Other compilers might work with small or no modifications of the build system and/or the source code.
...
\begin{itemize}
  ...
  This function returns the order of supplied derivatives. In case of $0$ the \fun{Sys::deri(...)} can be empty, although should be defined. For $1$ only the first order derivatives with respect to the state variables and the parameters need to be supplied. If the returned value is $2$ all the required derivatives are supplied by the user. The internal computation technique of the derivatives is the same as in DDE-BIFTOOL, which is the finite difference method. The derivatives are computed recursively, so the second order derivatives are calculated using the first order derivatives.
