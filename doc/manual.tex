...
%
\item[\funp{TYPE}] ~\\
Specifies the type of the solution to be continued. 
The bifurcation points are either defined by characteristic matrices (\funp{1}-\funp{5}, \funp{11}-\funp{16}) or by test functionals (\funp{41}-\funp{45}, \funp{51}-\funp{56}). It is recommended to use the algorithms with test functionals, because they are faster and usually more reliable than the characteristic matrices. In case of any difficulties try both methods before reporting the problem.
In the following the possible algorthms are listed. After a description the number in parentheses refer to the required number of additional free parameters.

Algorithms for time-periodic (non-autonomous) systems with characteristic matrices:
\begin{description}
...
%
\item[\funp{4} -] Switches to the other solution at a branching point and follows the orbits (0). Similar to \funp{TYPE=1}, first the software converges to the critical solution. Then it computes the critical eigenfunction $\varphi^{cr}$, which will become the tangent of the new branch, so that the continuation can start.
%
\item[\funp{5} -] The software first switches to the period-2 branch at a period doubling bifurcation point
...
%
\item[\funp{14} -] Switches to the other solution at a branching point and follows the orbits (1).
%
\item[\funp{15} -] The software first switches to the period-2 branch at a period doubling bifurcation point
...
\end{description}
The algorithms with test functionals are similarly numbered. For time-periodic systems:
\begin{description}
